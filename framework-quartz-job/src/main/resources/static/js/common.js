let common = {
    ...
    okHandle: function (json, index, tableId, ok) {
        if (json.code === common.status.ok) {
            let okMsg = ok !== undefined ? ok : json.message;
            layer.msg(okMsg);
            if (index)
                layer.close(index);
            if (tableId)
    ...
    dateFormatter: function (date, split, show) {
        let realLen = arguments.length;
        let symbol = realLen >= 2 ? arguments[1] : "-";
        let showTime = realLen === 3 ? arguments[2] : true;
        let resD = '';
    ...
    hashChange: function ($) {
        ...
        let restIndex = hash.indexOf("!!");
        if (restIndex > -1) {
            hash = hash.substring(0, restIndex);
        }
        let $layuiThis = $("li.layui-nav-item a[href=" + hash + "]");
        if ($layuiThis !== undefined && $layuiThis.length > 0) {
            let $layItem = $layuiThis.last();
    ...
    stickyIt: function () {
        ...
        let isNotWebkit = (agent.indexOf("Edge/") > -1) || (agent.indexOf("Firefox/") > -1);
        if (isNotWebkit) {
            if ($("#main-body")) {
    ...
    sticky: function () {
        let listHeight = $(".layui-container>.layui-row>.layui-col-md9").outerHeight(true);
        let stickHeight = $(".layui-container #affix-side").parents(".layui-col-md3").outerHeight(true);
